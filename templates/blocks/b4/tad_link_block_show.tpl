    ...
    <div class="row">
        ...
        <div class="col-sm-<{$block.col}>" <{if $block.height}>style="height: <{$block.height}>px;"<{/if}>>
            <div class="thumbnail my-2" <{if $block.height!=0}>style="height: <{$block.height}>px; overflow: hidden;"<{/if}>>
                ...
                <{if $web.title or $web.url}>
                    <div class="caption">
                        <{if $web.title}><p style="font-size: 0.75em;height: 18px; overflow: hidden;"><{$web.title}></p><{/if}>
                        <{if $web.url}><p style="font-size: 0.6875em; height: 18px; overflow: hidden;"><{$web.url}></p><{/if}>
                    </div>
                <{/if}>
            </div>
        </div>
